export function watchSysCall(){
    var base;
    var do_dlopen:NativePointer = NULL;
    var call_ctor:NativePointer = NULL;
//Module.load('/data/data/com.sbi.lotusintouch/execve_arm64.so');
    ...
    function consoleLog(text: string) {
        const dt = new Date();
        const message = `[${dt.toUTCString().replace(" GMT", "")}.${dt.getMilliseconds()}] ${text}`;
        console.log(message);
    }
                ...
                mod.enumerateRanges('--x').forEach(function (range) {
                    Memory.scanSync(range.base, range.size, '01 00 00 d4').forEach(function (match) {
                        Interceptor.attach(match.address, function () {
                            ...
                            //consoleLog('syscall ',table[sc],'['+sc+']');
                            if(sc == 0x38) {
                                // @ts-ignore
                                const msg = md.findName(this.returnAddress)+' : syscall openat("'+this.context['x1'].readUtf8String()+'",'+this.context['x2']+','+this.context['x3']+')';
                                consoleLog(msg);
                            } else {
                                // @ts-ignore
                                consoleLog(md.findName(this.returnAddress)+' : syscall ',table[sc],'['+sc+']');
                            }
                        });
                    });
                });
